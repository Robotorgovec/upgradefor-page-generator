...
export default function SportpitShell({ children }: PropsWithChildren) {
  ...
  // determine if user is currently in USA context (path or query)
  const isUsaContext =
    pathname.startsWith("/catalog/usa") || searchParams.get("origin") === "USA";

  // compute navigation open state for burger (mobile vs desktop)
  ...
  const onCountryChange = (value: string) => {
    ...
    if (isUsaContext) {
      const query = new URLSearchParams(searchParams.toString());
      // clear origin and catalog-related parameters
      query.delete("origin");
      query.delete("cat");
      query.delete("sub");
      query.delete("brand");
              ...
              <div className={styles.ratingRow}>
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    onClick={() =>
                      setRatingMin((prev) => (prev === star ? null : star))
                    }
                  >
                    ★
                  </button>
