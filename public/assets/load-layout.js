(async function () {
  ...
  console.log("[UPGR] load-layout v20260125-2 loaded");
  ...
  function updateSidebarFooterSpacing(sidebar, inner, footer) {
    const footerHeight = footer ? footer.offsetHeight : 0;
    sidebar.style.setProperty("--sidebar-footer-offset", `${footerHeight}px`);
    if (!footerHeight) {
      inner.style.removeProperty("padding-bottom");
    }
  }

  function removeSidebarFooter(sidebar) {
    const footer = sidebar.querySelector(":scope > .sidebar-footer");
    if (footer) footer.remove();
    sidebar.style.removeProperty("--sidebar-footer-offset");
    const inner = qs(".sidebar-inner", sidebar);
    if (inner) inner.style.removeProperty("padding-bottom");
  }

  function initStickyFooter() {
    const sidebar = qs(".sidebar");
    if (!sidebar) {
      console.warn("[UPGR] sidebar not found");
      return;
    }

    if (window.innerWidth > mobileBreakpoint) {
      removeSidebarFooter(sidebar);
      return;
    }

    const inner = qs(".sidebar-inner", sidebar);
    if (!inner) {
      console.warn("[UPGR] sidebar-inner not found");
      return;
    }
    ...
    getSessionSafe().then((session) => {
      ...
      requestAnimationFrame(() => updateSidebarFooterSpacing(sidebar2, inner2, footer2));
    });

    requestAnimationFrame(() => updateSidebarFooterSpacing(sidebar, inner, footer));
    console.log("[UPGR] account footer created");
  }

  try {
    await fetchAndInsert("/includes/header.html", "header");
    console.log("[UPGR] header inserted");
    await fetchAndInsert("/includes/menu.html", ".sidebar");
    console.log("[UPGR] menu inserted");
