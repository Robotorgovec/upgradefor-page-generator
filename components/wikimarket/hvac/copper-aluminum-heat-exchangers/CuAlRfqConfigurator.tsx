...
function buildPrefillMessage(productSlug?: string, usecaseSlug?: string): string {
  const parts: string[] = [];

  if (productSlug) {
    parts.push(`изделие: ${slugToReadableText(productSlug)}`);
  }

  if (usecaseSlug) {
    parts.push(`применение: ${slugToReadableText(usecaseSlug)}`);
  }
  ...
  return `Предзаполнено: ${parts.join(" · ")}. Проверьте выделенные поля ниже.`;
}
...
export default function CuAlRfqConfigurator({
  prefillProductSlug,
  prefillUsecaseSlug,
  prefillSignal,
}: CuAlRfqConfiguratorProps) {
  ...
  const [prefillHighlight, setPrefillHighlight] = useState({
  ...
  useEffect(() => {
    ...
    setForm((prev) => {
      const productLine = prefillProductSlug ? `Интересует изделие: ${slugToReadableText(prefillProductSlug)}` : "";
      const nextComment =
        productLine && !prev.comments.includes(productLine)
          ? [prev.comments.trim(), productLine].filter(Boolean).join("\n")
          : prev.comments;
    ...
    setPrefillMessage(buildPrefillMessage(prefillProductSlug, prefillUsecaseSlug));
    setPrefillHighlight({
      applicationArea: Boolean(prefillUsecaseSlug),
      comments: Boolean(prefillProductSlug),
    });
  }, [prefillProductSlug, prefillSignal, prefillUsecaseSlug]);
  ...
  const setField = <K extends keyof RfqFormState>(key: K, value: RfqFormState[K]) => {
    ...
    if (key === "applicationArea") {
      setPrefillHighlight((prev) => ({ ...prev, applicationArea: false }));
    }
    if (key === "comments") {
      setPrefillHighlight((prev) => ({ ...prev, comments: false }));
    }
  };
    ...
    try {
      if (endpoint) {
        ...
        setSubmitState({
          status: "success",
          message:
            "На этой preview-версии финальная отправка ещё не подключена. Заполненные данные сохранены локально в браузере, чтобы вы не потеряли заполнение.",
          stopBoundary: true,
        });
      }
    ...
    if (step === 0) {
      return (
        <section className={styles.stepSection} aria-labelledby="rfq-step-1">
          ...
          <label className={`${styles.field} ${prefillHighlight.applicationArea ? styles.fieldPrefilled : ""}`}>
    ...
    if (step === 4) {
      return (
        <section className={styles.stepSection} aria-labelledby="rfq-step-5">
          ...
          <label className={`${styles.field} ${prefillHighlight.comments ? styles.fieldPrefilled : ""}`}>
      ...
      return (
        <section className={styles.stepSection} aria-labelledby="rfq-step-6">
          ...
          <div className={styles.routingBlock}>
            ...
            <p className={styles.microHelp}>
              После отправки будет использован выбранный способ обработки заявки.
            </p>
          </div>
        </section>
      );
    ...
    return (
      <section className={styles.stepSection} aria-labelledby="rfq-step-7">
        ...
        <div className={styles.reviewGrid}>
          ...
          <article className={styles.reviewCard}>
            ...
            <ul>
              ...
              <li>Способ обработки: {ROUTING_PREFERENCE_OPTIONS.find((item) => item.value === form.routingPreference)?.label}</li>
            </ul>
          </article>
        </div>
        ...
        <label className={styles.checkboxLine}>
          ...
          <span>Согласен на обработку данных для подбора, оценки и подготовки коммерческого предложения.</span>
        </label>
      </section>
    );
  };
  ...
  return (
    <section id="request" className={styles.configurator} data-configurator-view="true">
      <div className={styles.header}>
        ...
        {prefillMessage ? (
          <p className={styles.highlightNotice} role="status" aria-live="polite">
            {prefillMessage}
          </p>
        ) : null}
      </div>
      ...
      <div className={styles.layout}>
            ...
            <div className={styles.successState} role="status">
              ...
              {submitState.stopBoundary ? (
                <p className={styles.stopBoundaryNote}>
                  После подключения рабочего канала отправки эта кнопка будет передавать данные напрямую в обработку без повторного заполнения формы.
                </p>
               ) : null}
            </div>
           ) : null}
        ...
        <aside className={styles.summaryColumn}>
          ...
          >
            {mobileSummaryOpen ? "Скрыть сводку" : "Показать сводку"}
          </button>
          ...
          <div className={`${styles.summarySticky} ${mobileSummaryOpen ? styles.summaryOpen : ""}`}>
            ...
            <article className={styles.estimateCard} data-event="estimate_view">
              ...
              <p className={styles.estimateHint}>Диапазон шире, если часть исходных данных пока не заполнена.</p>
              <p className={styles.estimateHint}>{DEFAULT_ESTIMATE_TEXT.precisionHint}</p>
            </article>

            <article className={styles.estimateCard}>
              <h3>Как повысить точность оценки</h3>
              <ul>
                <li>Добавьте температуры, расходы и режим работы по воздуху и внутренней среде.</li>
                <li>Укажите габариты, ограничения по подключению и требования к посадке.</li>
                <li>Приложите чертёж, фото шильдика или снимок старой секции.</li>
                <li>Сразу отметьте нужные документы, покрытие и желаемый срок поставки.</li>
              </ul>
            </article>
          </div>
        </aside>
