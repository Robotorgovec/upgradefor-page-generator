...
export const DEFAULT_MANUFACTURER_CARD_IMAGE = "/assets/logo/logo-blue-only.png";

export const DEFAULT_MANUFACTURER_CARD_IMAGE_ALT = "Каталог производителей Cu-Al";
...
export function getCompanyImage(card: Pick<ManufacturerCompanyCardView, "logoUrl" | "primaryImageUrl">): string {
  return hasValue(card.logoUrl) ? card.logoUrl : DEFAULT_MANUFACTURER_CARD_IMAGE;
}

export function getCompanyImageAlt(
  card: Pick<ManufacturerCompanyCardView, "logoUrl" | "name" | "primaryImageAlt">,
): string {
  return hasValue(card.logoUrl) ? `${card.name} logo` : `Карточка ${card.name}`;
}
...
export function getVerificationHint(
  card: Pick<ManufacturerCompanyCardView, "isVerified" | "hasCertificates" | "hasTechnicalDocs">,
): string {
  if (!card.isVerified) {
    return "Поставщик показан в каталоге, но комплект документов ещё не подтверждён.";
  }

  if (card.hasCertificates && card.hasTechnicalDocs) {
    return "Проверен: подтверждены документы, техданные и статус компании по категории Cu-Al.";
  }

  if (card.hasCertificates || card.hasTechnicalDocs) {
    return "Проверен: часть документов подтверждена, остальное уточняется по запросу.";
  }

  return "Проверен: статус компании подтверждён для работы по Cu-Al проектам.";
}

export function getTrustEvidence(
  card: Pick<
    ManufacturerCompanyCardView,
    "hasCertificates" | "hasTechnicalDocs" | "supportsReplacementBySample" | "responseSpeedLabel" | "leadTimeLabel"
  >,
): string[] {
  const evidence: string[] = [];

  if (card.hasCertificates) {
    evidence.push("Документы подтверждены");
  }

  if (card.hasTechnicalDocs) {
    evidence.push("Есть чертежи и техданные");
  }

  if (card.supportsReplacementBySample) {
    evidence.push("Подбор по образцу и шильдику");
  }

  if (evidence.length < 3 && hasValue(card.responseSpeedLabel)) {
    evidence.push(card.responseSpeedLabel);
  }

  if (evidence.length < 3 && hasValue(card.leadTimeLabel)) {
    evidence.push(`Срок: ${card.leadTimeLabel}`);
  }
